...
class Matcher():
    ...
    def __init__(self, image1: np.ndarray, image2: np.ndarray, method: Enum=Method.ORB, threshold=800) -> None:
        """输入两幅图像，计算其特征值
    ...
    def match(self, max_match_lenth=20, threshold=0.04, show_match=False):
        ...
        if show_match:
            ...
            # show_image(img3)
            cv2.imwrite('../example/3-sift-match.jpg', img3)
...
class Stitcher:
    ...
    def __init__(self, image1: np.ndarray, image2: np.ndarray, method: Enum=Method.ORB, use_kmeans=False):
        """输入图像和匹配，对图像进行拼接
        目前采用简单矩阵匹配和平均值拼合
    ...
    def stich(self, show_result=False, max_match_lenth=40, show_match_point=False, use_partial=False, use_new_match_method=False, use_gauss_blend=True):
        """对图片进行拼合
...
if __name__ == "__main__":
    ...
    img1 = cv2.imread("../example/19-left.JPG")
    img2 = cv2.imread("../example/19-right.JPG")
